use crate::{
    ...
    state::{Tick, TickUpdate, PoolRewardInfo},
};
...
mod tick_manager_tests {
    ...
    use crate::{
        ...
        math::Q64_RESOLUTION,
        state::{tick_builder::TickBuilder, Tick, PoolRewardInfo, NUM_REWARDS},
    };
    ...
    fn test_next_tick_modify_liquidity_update() {
        ...
        // Pool rewards re-used in the tests
        let reward_infos = [
            PoolRewardInfo {
    ...
    fn test_next_tick_modify_liquidity_update_errors() {
        ...
        for test in [
            ...
            let err = next_tick_modify_liquidity_update(
                ...
                0,
                0,
                &[PoolRewardInfo::default(); NUM_REWARDS],
                test.liquidity_delta,
                test.is_upper_tick,
            )
            .unwrap_err();
